...
namespace WindowsPet.Models
{
    public class FileManager : IFileManager
    {
        ...
        public FileManager(IPetService petService, IPetRepository petRepository, HttpClient? httpClient = null)
        {
        ...
        // Get All Files in Pet Directory
        public async Task<List<string>> GetAllFilesInFolderAsync(string petId)
        {
            ...
            {
                Console.WriteLine($"GetAllFilesInFolderAsync Error: {ex.Message}");
                return new List<string>();
            }
        }
        ...
        public async Task DownloadFileAsync(string url, string savePath, string filename)
        {
            try
            {
                var response = await _httpClient.GetAsync($"http://{RestApiSetting.IpAddress}:{RestApiSetting.Port}/api/file/download?name={url}");
        ...
        public List<string> GetAllFileFromDirectory(Guid petId)
        {
            try
            {
                var pet = _petRepository.GetById(petId);
                var categoryType = pet?.PetCategories?.FirstOrDefault()?.Type ?? "";
                var targetDir = Path.Combine(LocalStorageSetting.LocalCache, categoryType, petId.ToString());
                if (Directory.Exists(targetDir))
                {
                    return Directory.GetFiles(targetDir).ToList();
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"GetAllFileFromDirectory Error: {ex.Message}");
            }
            return new List<string>();
        }

        public List<string> GetAllGIFFileFromDirectory(Guid petId)
        {
            try
            {
                var pet = _petRepository.GetById(petId);
                ...
                if (Directory.Exists(targetDir))
                {
                    return Directory.GetFiles(targetDir).Where(u => u.EndsWith(".gif", StringComparison.OrdinalIgnoreCase)).ToList();
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"GetAllGIFFileFromDirectory Error: {ex.Message}");
            }
            return new List<string>();
        }

        public string GetSpecificFileUri(Guid petId)
        {
            return $"http://{RestApiSetting.IpAddress}:{RestApiSetting.Port}/api/file/downloadspecific?name={petId}";
        }

        public static class RestApiSetting
        {
            public readonly static string IpAddress = "192.168.0.104";
            public readonly static int Port = 5225;
        }

        public static class LocalStorageSetting
        {
        ...
        public static class FileUriSetting
        {
