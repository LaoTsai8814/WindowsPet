    ...
    public class PetService: IPetService
    {
        ...
        public void AddPetListToUser(Guid token, List<Pet> pets)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        var trackedPet = petRepository.GetById(pet.Id);
                        if (trackedPet != null)
                        {
        ...
        public void AddPopularPetToTable(List<Pet>? petList)
        {
            ...
            try
            {
                ...
                {
                    // 檢查資料庫中是否已存在該寵物
                    var existingPet = petRepository.GetById(pet.Id); // 這裡假設 GetById 方法會返回 null 如果找不到該寵物
        ...
        public bool IsPetOwnByUser(Guid UserId,Guid PetId)
        {
            try
            {
                var pet = userRepository.GetByToken(UserId).UserPets?.FirstOrDefault(p => p.Id == PetId);
                if (pet == null)
                {
                    return false;
                }
        ...
        public List<Pet> GetPetsByCategory(Pet.PetCategorySet PetCategory)
        {
            try
            {
